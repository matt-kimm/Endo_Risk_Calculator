...
with tab2:
    ...
    st.caption("Дополнительные аналитические инструменты для изучения введённых данных.")
        ...
        # Секция интерактивного обучения моделей
    with st.expander("🤖 Обучение моделей на ваших данных", expanded=st.session_state['expander_states']['training']):
                            ...
                            if len(X) < 20:
                                ...
                                if not models_to_train:
                                    ...
                                    if metrics_list:
                                        ...
                                        st.session_state['model_f1'] = metrics_df.loc[selected_model, 'F1']  # Добавьте эту строку
                                        st.session_state['best_metric'] = best_metric  # Добавьте эту строку
        ...
        if st.session_state.get('model_trained', False):
            ...
            if best_metric == 'PR-AUC' and model_pr_auc < 0.3:
                ...
                # Для F1 можно добавить проверку, если метрика сохранена
                pass
            ...
            if X_test_data is not None:
                ...
                if st.button("🔮 Предсказать", key="predict_button"):
                    try:
                        ...
                        risk_factors = []
                        if test_input.get('age', 0) > 60:
                            risk_factors.append("Возраст > 60 лет")
                        if test_input.get('bmi', 0) > 30:
                            risk_factors.append("Ожирение (BMI > 30)")
                        if test_input.get('glucose', 0) > 6.0:
                            risk_factors.append("Повышенная глюкоза")
                        if test_input.get('family_diabetes', 0) == 1:
                            risk_factors.append("Семейная история диабета")
                        if test_input.get('hypertensive', 0) == 1:
                            risk_factors.append("Гипертония")
                        if test_input.get('cardiovascular_disease', 0) == 1:
                            risk_factors.append("Сердечно-сосудистые заболевания")
                        
                        if risk_factors:
                            st.warning("Обнаружены факторы риска: " + ", ".join(risk_factors))
                        
                        # Интерпретация
                        if probability[1] > 0.5:
                            st.error("Высокая вероятность положительного класса.")
                        elif probability[1] > threshold:
                            st.warning(f"Вероятность выше порога ({threshold:.2f}). Рекомендуется дополнительная проверка.")
                        elif probability[1] > 0.2:
                            st.info("Умеренная вероятность. Учитывая факторы риска, рекомендуется консультация врача.")
                        else:
                            st.success("Низкая вероятность положительного класса.")
                        
                        if len(risk_factors) >= 3 and probability[1] < 0.3:
                            st.error("⚠️ Внимание: несмотря на низкую вероятность по модели, наличие множественных факторов риска требует медицинской консультации.")
    ...
    # Собираем числовые показатели текущего пациента
    numeric_data = {
        "Возраст": age,
        "ИМТ": bmi,
        "Талия": waist_cm,
        "Сон": sleep_hours,
        "Глюкоза": fasting_glucose if fasting_glucose > 0 else np.nan,
        "HbA1c": hba1c if hba1c > 0 else np.nan,
        "ТТГ": tsh_value if tsh_value > 0 else np.nan,
        "Св.T4": ft4_value if ft4_value > 0 else np.nan,
        "Кальций": serum_calcium if serum_calcium > 0 else np.nan,
    }
    # Добавляем баллы рисков
    risk_scores = {
        "Диабет": diabetes_score,
        "ИР": ir_score,
        "Гипотиреоз": hypothyroid_score,
        "Гипертиреоз": hyperthyroid_score,
        "PCOS": pcos_score if pcos_score is not None else np.nan,
        "Кости": bone_score,
        "Кушинг": cushing_rule_score,
        "Аддисон": addison_rule_score,
        "Гиперпаратиреоз": hyperpara_rule_score,
        "Сеть": network_score,
    }
    numeric_data.update(risk_scores)

    numeric_df = pd.DataFrame([numeric_data])
    numeric_df = numeric_df.dropna(axis=1)

    if numeric_df.shape[1] >= 2:
        st.write("### Показатели текущего пациента")
        
        # Преобразуем в длинный формат
        numeric_long = numeric_df.melt(var_name='Показатель', value_name='Значение')
        
        # Нормализуем для визуализации
        numeric_long['Нормализовано'] = (numeric_long['Значение'] - numeric_long['Значение'].min()) / \
                                        (numeric_long['Значение'].max() - numeric_long['Значение'].min() + 1e-10) * 100
        
        # Горизонтальная столбчатая диаграмма
        fig_bar, ax_bar = plt.subplots(figsize=(10, max(4, len(numeric_long) * 0.4)))
        numeric_long_sorted = numeric_long.sort_values('Значение', ascending=True)
        colors = plt.cm.RdYlGn_r(numeric_long_sorted['Нормализовано'] / 100)
        
        ax_bar.barh(numeric_long_sorted['Показатель'], 
                    numeric_long_sorted['Нормализовано'],
                    color=colors, alpha=0.8)
        
        for i, (idx, row) in enumerate(numeric_long_sorted.iterrows()):
            ax_bar.text(row['Нормализовано'] + 1, i, 
                        f"{row['Значение']:.1f}", 
                        va='center', fontsize=8)
        
        ax_bar.set_xlabel('Нормализованное значение (%)')
        ax_bar.set_title('Сравнение числовых показателей (нормализовано)')
    ...
    # Сохраняем данные рядов при изменении
    if 'mfdfa_series_list' in st.session_state:
        for i, item in enumerate(st.session_state['mfdfa_series_list']):
            if f"mfdfa_data_{i}" in st.session_state:
                item['data'] = st.session_state[f"mfdfa_data_{i}"]
            if f"mfdfa_name_{i}" in st.session_state:
                item['name'] = st.session_state[f"mfdfa_name_{i}"]
    ...
    if st.button("🔍 Анализировать ряды", key="analyze_mfdfa", type="primary"):
        ...
        for item in st.session_state['mfdfa_series_list']:
            ...
            if arr is not None and len(arr) >= 12:
                ...
                if item['data'].strip():  # Если есть данные, но недостаточно
    ...
    # ======================== РАСШИРЕННАЯ АНАЛИТИКА ДЛЯ ВРАЧА ========================
    st.markdown("---")
    st.header("📊 Расширенная аналитика для врача")
    st.caption("Дополнительные инструменты для углублённого анализа данных пациента и загруженных наборов.")
    ...
    if not st.session_state.get('report_generated', False):
        ...
        if not results:
            ...
            # Словарь референсных диапазонов (значения могут быть адаптированы)
            reference_ranges = {
                "Глюкоза натощак (мг/дл)": (70, 99),       # норма 70-99
                "HbA1c (%)": (4.0, 5.6),
                "ТТГ (мМЕ/л)": (0.4, 4.0),
                "Св. T4 (нг/дл)": (0.8, 1.8),
                "Кальций общий (мг/дл)": (8.5, 10.2),
                "ИМТ": (18.5, 24.9),
                "Талия (см)": (None, 94 if gender == 0 else 80),  # для мужчин/женщин
                "Сон (часов)": (7, 9),
                "Возраст (лет)": (18, 65),  # просто для справки, не строго
            }
            ...
            # 5. ПРОЦЕНТИЛИ ПАЦИЕНТА ОТНОСИТЕЛЬНО ВЫБОРКИ
